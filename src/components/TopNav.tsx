...
export default function TopNav({navigations}: ITopNavProps) {
    return (
        <div id="nav" className="flex flex-col justify-between px-12 lg:px-64 md:px-36 py-2 items-center md:flex-row gap-y-2">
            <div>
                <Image src="/logo.png" alt="Hygiene Angel Logo" width={16 * 8.2} height={9 * 8.2} priority={true}
                       className="w-auto h-auto"/>
            </div>
            <nav className="flex-row space-x-8 lg:space-x-28 ">
                {navigations.map((key) => (
                    <a key={key} href={`#${key}`}
